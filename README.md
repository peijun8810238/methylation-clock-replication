...
## ご依頼・コラボレーションについて（日本語）

本リポジトリの内容に関連して、以下のような 業務・研究のご相談をお受けしています。
- DNAメチル化データ解析（Illumina 450K / EPIC）
- エピジェネティッククロック（Horvath など）の実装・検証
- Python × R を用いた再現可能な解析パイプライン構築
- GEO 等の公開オミクスデータの前処理・解析
- 研究コードの整理・自動化・ドキュメント化

本リポジトリは、
論文モデルの忠実な再現と 実務を意識した設計の両立を意識して作成しています。

ご興味があれば、お気軽にご連絡ください。

連絡先: jumpei.taniguchi.work@gmail.com
GitHub Issue / Discussion 経由でも問題ありません。